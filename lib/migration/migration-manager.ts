...
class ImageMigrationManager {
  ...
  constructor(options: Partial<MigrationConfig> = {}) {
    ...
    this.logger = Logger.getInstance(
      this.config.verbose ? LogLevel.VERBOSE : LogLevel.INFO
    );

    // Initialize components
    this.strapiClient = new StrapiMediaClient(this.config);
    this.imageProcessor = new ImageProcessor(this.config);
    this.backupManager = new BackupManager(this.config, this.strapiClient);
   ...
   */
  async migrate(): Promise<ProcessingResult> {
    ...
    try {
      ...
      this.logger.failure('Migration failed', { error: (error as Error).message });
      throw error;
    }
  }
   ...
   */
  private async fetchMenuItems(): Promise<any[]> {
   ...
   */
  private async createBackup(menuItems: any[]): Promise<void> {
   ...
   */
  private async processMigration(menuItems: any[]): Promise<ProcessingResult> {
   ...
   */
  private async processItemsConcurrently(items: any[]): Promise<ProcessingResult> {
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      for (let i = 0; i < batchResults.length; i++) {
        ...
        this.logger.progress(
          result.processed, 
          items.length, 
          `${item.name} - ${result.successful}✅ ${result.failed}❌ ${result.skipped}⏭️`
        );
      }
    }
   ...
   */
  private async processMenuItem(item: any): Promise<UploadResult> {
    try {
      ...
      const uploadResult = await this.strapiClient.uploadImage(
        imageResult.fullPath!,
        {
      ...
      if (!uploadResult.skipped && uploadResult.mediaId) {
        const updateSuccess = await this.strapiClient.updateMenuItemImage(
          item.documentId,
   ...
   */
  private generateMigrationReport(result: ProcessingResult): void {
   ...
   */
  private generateRecommendations(result: ProcessingResult): void {
  ...
  async rollback(backupPath?: string): Promise<boolean> {
    try {
      ...
      this.logger.error('Rollback failed', { error: (error as Error).message });
      return false;
    }
  }
   ...
   */
  async validateOnly(): Promise<ValidationResult> {
...
module.exports = { ImageMigrationManager };