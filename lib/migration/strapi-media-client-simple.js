/**
 * Simple Strapi Media API Client
...
export class StrapiMediaClient {
  constructor(options) {
  ...
  async uploadImage(filePath, metadata = {}) {
    try {
      ...
      const blob = new Blob([fileBuffer], { type: this.getMimeType(path.extname(fileName)) });
      ...
      delete authHeaders['Content-Type']; // Let FormData set this
